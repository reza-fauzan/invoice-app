...
@section('content')
    ...
    <div class="page-header">
        <div>
            <h1>Edit Invoice</h1>
            <p>Perbarui data invoice <strong>{{ $invoice->nomor_invoice }}</strong>.</p>
        </div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px;">
                ...
                <div>
                    ...
                    <select name="status_pembayaran" id="status_pembayaran" class="form-input" required>
                        @foreach(['Draft', 'Unpaid', 'Paid', 'Canceled'] as $status)
                            <option value="{{ $status }}" {{ old('status_pembayaran', $invoice->status_pembayaran) == $status ? 'selected' : '' }}>{{ $status }}</option>
                        @endforeach
                    </select>
...
<script>
    ...
    @foreach($invoice->invoiceDetails as $detail)
        addItem({{ $detail->produk_id }}, {{ $detail->qty }});
    @endforeach
</script>
@endpush

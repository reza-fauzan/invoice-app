...
@section('content')
    <div class="page-header">
        <div>
            ...
            <p>Kelola master data merk kendaraan.</p>
        </div>
    ...
    <div class="table-container">
        <div class="table-header">
            <div>
                <h3>Daftar Merk Kendaraan</h3>
                <p class="table-header-sub">Total: {{ $items->count() }} merk</p>
            </div>
        </div>
        <table>
            <thead>
            ...
            <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display: flex; gap: 6px;">
                                <button type="button" class="btn-icon" title="Edit" onclick="openModal('edit', {{ $item->id }}, '{{ addslashes($item->nama) }}')">
                                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                        <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
                                        <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
                                    </svg>
                                </button>
                                <form action="{{ route('merk-kendaraan.destroy', $item) }}" method="POST" onsubmit="return confirm('Yakin hapus merk ini?')">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="3" style="text-align: center; padding: 48px 24px; color: var(--color-text-muted);">
                            <div style="font-size: 15px; font-weight: 600; color: var(--color-text-secondary); margin-bottom: 4px;">Belum ada data</div>
                            <div style="font-size: 13px;">Tambahkan merk kendaraan pertama</div>
                        </td>
                    </tr>
                @endforelse
            </tbody>
...
<div id="modalOverlay" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.5); backdrop-filter: blur(6px); -webkit-backdrop-filter: blur(6px); z-index: 9999; justify-content: center; align-items: center;" onclick="if(event.target===this) closeModal()">
    <div style="background: var(--color-surface); border-radius: 16px; padding: 32px; width: 100%; max-width: 440px; box-shadow: 0 20px 60px rgba(0,0,0,0.3); border: 1px solid var(--color-border); animation: modalIn 0.2s ease-out;">
        ...
        <form id="createForm" action="{{ route('merk-kendaraan.store') }}" method="POST" style="display: none;">
            @csrf
            <div style="margin-bottom: 20px;">
                <label class="form-label">Nama Merk <span style="color: var(--color-danger);">*</span></label>
                <input type="text" name="nama" class="form-input" placeholder="Masukkan nama merk kendaraan" required id="createInput">
            </div>
        ...
        <form id="editForm" action="" method="POST" style="display: none;">
            ...
            <div style="margin-bottom: 20px;">
                <label class="form-label">Nama Merk <span style="color: var(--color-danger);">*</span></label>
                <input type="text" name="nama" class="form-input" required id="editInput">
            </div>
            <div style="display: flex; gap: 10px; justify-content: flex-end;">
                <button type="button" class="btn btn-secondary" onclick="closeModal()">Batal</button>
